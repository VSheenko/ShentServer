...
void AuthRepository::add_refresh_token(int user_id, std::string token, int ttl_sec) {
	redis_client_->set(std::format("refresh:{}", user_id), token, ttl_sec);
}
...
void AuthRepository::ban_refresh_token(int user_id, int ttl_sec) {
	...
	std::optional<std::string> token_opt = get_refresh_token(user_id);
...
bool AuthRepository::validate_refresh_token(int user_id, std::string token) {
	...
	std::optional<std::string> unban_opt = get_refresh_token(user_id);
...
std::optional<std::string> AuthRepository::get_refresh_token(int user_id) {
